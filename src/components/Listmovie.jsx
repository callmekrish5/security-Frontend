...
function Listmovie() {
    ...
    async function listmovies(){
        ...
        try {
            //user list
            const fetchResponse = await fetch(`https://${location}:3000/movies/listallmovie/`, settings);
    ...
    async function deleteMovie(_id){
        if(window.confirm("Do you want to delete the movie?")){
            ...
            try {
            //delete movie
            const fetchResponse = await fetch(`https://${location}:3000/movies/delete/`, settings);
            const data = await fetchResponse.json();
            listmovies();
            return data;
            }
    ...
    async function updateMovie(_id){
        if(window.confirm("Do you want to edit the movie?")){
            ...
            try {
            //user list
            const fetchResponse = await fetch(`https://${location}:3000/movies/specificMovie/`, settings);
